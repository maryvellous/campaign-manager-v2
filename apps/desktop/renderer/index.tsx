import { GraphView, folderPalette, folderColor } from './graph-view';
import { defaultUi, type UiState } from '../application/workspace-types';
...
const defaults: UiState = defaultUi();
...
function App() {
  ...
  const [searchPending, setSearchPending] = useState(false); const [searchFailure, setSearchFailure] = useState<string>();
  ...
  const [compactSidebar, setCompactSidebar] = useState(false); const [compactInspector, setCompactInspector] = useState(false);
  ...
  async function command(input: Record<string, unknown>) {
    ...
    try {
      ...
      if (['note', 'newNote', 'activateTab', 'history'].includes(String(input.action))) { setCompactSidebar(false); }
  ...
  useEffect(() => {
    const listener = (event: KeyboardEvent) => {
      ...
      if ((event.ctrlKey || event.metaKey) && event.key.toLowerCase() === 's') { event.preventDefault(); void command({ action: stateRef.current?.ui.view === 'board' ? 'boardSave' : 'save' }); }
      ...
      if (event.key === 'Escape' && !palette) { setCompactSidebar(false); setCompactInspector(false); setWiki(undefined); setOperation(undefined); setTitleEditing(false); setDropTarget(undefined); }
  ...
  useEffect(() => {
    ...
    let canceled = false; setSearchPending(true); setSearchFailure(undefined);
    const timer = setTimeout(() => {
      void window.campaign.command({ action: 'search', query: trimmed }).then(reply => {
        if (canceled) return; setSearchPending(false);
        if (reply.ok && Array.isArray(reply.data)) setSearchResults(reply.data as typeof searchResults);
        else setSearchFailure(reply.error?.message ?? 'Ricerca non disponibile. Riprova a ricostruire l’indice.');
      }).catch(() => { if (!canceled) { setSearchPending(false); setSearchFailure('Ricerca non disponibile. Riprova a ricostruire l’indice.'); } });
    }, 120);
    return () => { canceled = true; clearTimeout(timer); };
  ...
  useEffect(() => {
    ...
    void window.campaign.command({ action: 'graph' }).then(reply => {
      if (!canceled && reply.ok && reply.data && typeof reply.data === 'object') {
  ...
  if (state?.campaign) commands.push(
    ...
    { id: 'rebuild-search', label: 'Ricostruisci indice ricerca', kind: 'azione', run: () => void command({ action: 'rebuildSearch' }) },
  ...
  const sidebarVisible = !panels.sidebarCollapsed && (viewport >= 760 || compactSidebar); const inspectorVisible = !panels.inspectorCollapsed && (viewport >= 1050 || compactInspector);
  return <div className="app" style={{ '--sidebar-width': `${panels.sidebarWidth}px`, '--inspector-width': `${panels.inspectorWidth}px` } as CSSProperties}>
    ...
    {!state?.campaign ? <main className="welcome"><span className="eyebrow">IL TUO MONDO, UNA PAGINA ALLA VOLTA</span><h1>Un posto per<br/>le tue storie.</h1><p>Apri una cartella di note Markdown.<br/>Le tue campagne restano sul tuo computer.</p><button className="primary" onClick={() => void command({ action: 'choose' })}>Apri cartella… <Icon name="forward" /></button>{!!state?.preferences.recent.length && <section className="recent-campaigns"><h2>Campagne recenti</h2>{state.preferences.recent.map(item => <button key={item.campaignId} onClick={() => void command({ action: 'recent', campaignId: item.campaignId })}><Icon name="folder" /><span><strong>{item.name}</strong><small>{item.path}</small></span><Icon name="forward" /></button>)}</section>}</main> : <main className="workspace">
      <nav className="rail" aria-label="Navigazione principale">{navigation.map(n => <button disabled={busy} className={view === n.id ? 'active' : ''} key={n.id} aria-label={n.label} aria-current={view === n.id ? 'page' : undefined} title={n.label} onClick={() => void command({ action: 'view', view: n.id })}><Icon name={n.icon} /><span>{n.label}</span></button>)}</nav>
      ...
      <section className="center"><div className="tabs" role="tablist" aria-label="Note aperte">{!sidebarVisible && <button className="icon-button" aria-label="Mostra sidebar" onClick={() => { setCompactSidebar(true); updatePanels({ sidebarCollapsed: false }); }}><Icon name="folder" /></button>}{state.tabs?.map((tab, index) => <div className={`tab ${tab.id === state.activeTabId ? 'active' : ''}`} key={tab.id} draggable onDragStart={e => e.dataTransfer.setData('application/x-campaign-tab', tab.id)} onDragOver={e => e.preventDefault()} onDrop={e => { e.preventDefault(); const id = e.dataTransfer.getData('application/x-campaign-tab'); if (state.tabs.some(t => t.id === id)) void command({ action: 'reorderTab', id, index }); }}><button role="tab" aria-selected={tab.id === state.activeTabId} onClick={() => void command({ action: 'activateTab', id: tab.id })} onKeyDown={e => { if (e.altKey && ['ArrowLeft', 'ArrowRight'].includes(e.key)) { e.preventDefault(); void command({ action: 'reorderTab', id: tab.id, index: Math.max(0, Math.min(state.tabs.length - 1, index + (e.key === 'ArrowRight' ? 1 : -1))) }); } }}><Icon name="note" size={14} /><span>{title(tab.document)}</span>{tab.document && tab.document.state !== 'clean' && <span className="unsaved" aria-label="Modifiche non salvate">•</span>}</button><button className="tab-close" aria-label={`Chiudi tab ${title(tab.document)}`} onClick={() => void command({ action: 'closeTab', id: tab.id })}><Icon name="close" size={12} /></button></div>)}<button className="icon-button" aria-label="Nuova nota in nuova tab" onClick={() => void command({ action: 'newNote', newTab: true })}><Icon name="plus" size={16} /></button><span className="tab-spacer" />{!inspectorVisible && <button className="icon-button" aria-label="Mostra inspector" onClick={() => { setCompactInspector(true); updatePanels({ inspectorCollapsed: false }); }}><Icon name="panel" /></button>}</div>
        ...
        {view === 'board' ? <BoardView key={state.activeBoard?.boardId ?? "boards"} disk={state.boardDisk} board={state.activeBoard} boards={state.boards} status={state.boardState} error={state.boardError} onCommand={command} onRawCommand={rawCommand} /> : ['notes', 'recent', 'favorites'].includes(view) ? doc ? <>
        ...
        </> : <div className="empty-document"><span className="empty-mark">✦</span><h1>La campagna è aperta.</h1><p>Scegli una nota o comincia una nuova pagina.</p><button className="primary" onClick={() => void command({ action: 'newNote' })}><Icon name="plus" size={16} />Nuova nota</button></div> : view === 'search' ? <section className="search-view"><div className="view-header"><h1>Ricerca</h1><button className="icon-button" aria-label="Ricostruisci indice" title="Ricostruisci indice" onClick={() => void command({ action: 'rebuildSearch' })}><Icon name="refresh" /></button></div><label className="search-box"><Icon name="search" size={16} /><input aria-label="Cerca note e contenuti" placeholder="Digita per cercare nelle note" value={searchQuery} onChange={e => setSearchQuery(e.target.value)} /></label>{(searchFailure || state.searchError) && <p role="alert">{searchFailure || state.searchError}</p>}{searchQuery.trim() && searchPending ? <p role="status">Ricerca in corso…</p> : searchQuery.trim() ? <div className="search-results">{searchResults.length ? searchResults.map(result => <button key={result.noteId} className="search-result" onClick={() => void command({ action: 'note', noteId: result.noteId })}><span className="match-kind">{result.matchKind}</span><strong>{result.title}</strong><small>{result.relativePath}</small>{result.snippet && <p>{result.snippet}</p>}</button>) : <p className="empty-list">Nessun risultato per “{searchQuery.trim()}”.</p>}</div> : <div className="empty-list">Digita per cercare nelle note.</div>}</section> : view === 'graph' ? <GraphView key={state.campaign.campaignId} data={graphData} folders={folders.map(f => f.id)} folderColors={ui.folderColors} preferences={ui.graph} onPreferences={graph => { void window.campaign.command({ action: 'ui', patch: { graph } }); }} onOpen={noteId => void command({ action: 'note', noteId })} /> : view === 'settings' ? <section className="settings-view"><span className="eyebrow">IL TUO SPAZIO DI LAVORO</span><h1>Impostazioni</h1><section><h2>Campagna locale</h2><p className="local-path">{state.campaign.root}</p><div className="actions"><button onClick={() => void command({ action: 'revealRoot' })}>Apri in Esplora file</button><button onClick={() => updatePanels({ sidebarWidth: 248, inspectorWidth: 265, sidebarCollapsed: false, inspectorCollapsed: false })}>Ripristina disposizione</button><button onClick={() => void command({ action: 'closeCampaign' })}>Chiudi campagna</button></div></section><section><h2>Ricerca locale</h2><p>Indice: {state.searchStatus ?? 'missing'}</p>{state.searchError && <p role="alert">{state.searchError}</p>}<button onClick={() => void command({ action: 'rebuildSearch' })}>Ricostruisci indice ricerca</button></section><section><h2>Account</h2><p>Non serve un account per usare Campaign Manager.</p><p className="muted">Le funzioni online collegate a un account arriveranno più avanti.</p><span className="status-pill">In arrivo</span></section><small className="muted">Campaign Manager · V0.1</small></section> : <section className="placeholder-view"><Icon name={view === 'compendium' ? 'book' : 'search'} size={40} /><span className="status-pill">In arrivo</span><h1>{view === 'compendium' ? 'Compendio' : 'Ricerca'}</h1><p>{view === 'compendium' ? 'Qui potrai consultare l’enciclopedia e copiare le voci che ti servono nelle note della campagna.' : 'Qui potrai cercare nel contenuto delle note. Per trovare subito un file puoi usare il filtro nella sidebar o i comandi.'}</p><button onClick={() => void command({ action: 'view', view: 'notes' })}>Torna alle note</button></section>}
