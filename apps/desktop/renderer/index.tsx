...
type View = 'notes' | 'search' | 'graph' | 'boards' | 'live' | 'assistant' | 'compendium' | 'recent' | 'favorites' | 'settings';
...
declare global { interface Window { campaign: { beforeClose: (listener: () => void) => () => void; command: (input: Record<string, unknown>) => Promise<Reply>; subscribe: (listener: (state: ShellState) => void) => () => void; subscribeLive: (listener: (state: DesktopLiveState) => void) => () => void; subscribeAi: (listener: (state: AiState) => void) => () => void } } }
...
const navigation: { id: View; label: string; icon: IconName }[] = [{ id: 'notes', label: 'Note', icon: 'note' }, { id: 'search', label: 'Ricerca', icon: 'search' }, { id: 'graph', label: 'Grafo', icon: 'graph' }, { id: 'boards', label: 'Board', icon: 'board' }, { id: 'live', label: 'Live', icon: 'live' }, { id: 'assistant', label: 'Assistente', icon: 'spark' }, { id: 'compendium', label: 'Compendio', icon: 'book' }, { id: 'recent', label: 'Recenti', icon: 'clock' }, { id: 'favorites', label: 'Preferiti', icon: 'star' }, { id: 'settings', label: 'Impostazioni', icon: 'settings' }];
...
function App() {
  ...
  const [activeBoardPath, setActiveBoardPath] = useState<string>();
  ...
  function remember() {
    ...
    if (editor.current && currentId.current) {
      const start = editor.current.selectionStart; const end = editor.current.selectionEnd;
      positions.current.set(currentId.current, { start, end, scroll: editor.current.scrollTop });
      setSelectedText(start === end ? '' : editor.current.value.slice(start, end));
    }
  }
  ...
  useEffect(() => {
    setAiContext({ kind: 'campaign' });
    void window.campaign.command({ action: 'ai:state' }).then(reply => { if (reply.ok && reply.data) setAi(reply.data as AiState); });
  ...
  useEffect(() => {
    const listener = (event: KeyboardEvent) => {
      ...
      if ((event.ctrlKey || event.metaKey) && event.key.toLowerCase() === 's' && !['boards', 'live', 'assistant'].includes(stateRef.current?.ui.view ?? '')) { event.preventDefault(); void command({ action: 'save' }); }
      if ((event.ctrlKey || event.metaKey) && event.key.toLowerCase() === 'e' && !['boards', 'assistant'].includes(stateRef.current?.ui.view ?? '')) { event.preventDefault(); void toggleReading(); }
  ...
  useEffect(() => {
    if (!state?.campaign || view !== 'search') { setSearchResults([]); return; }
    const trimmed = searchQuery.trim();
    if (!trimmed) { setSearchResults([]); return; }
    const timer = setTimeout(() => {
      void window.campaign.command({ action: 'search', query: trimmed }).then(reply => {
        if (reply.ok && Array.isArray(reply.data)) setSearchResults(reply.data as typeof searchResults);
      }).catch(() => setSearchResults([]));
    }, 120);
    return () => clearTimeout(timer);
  }, [view, state?.campaign?.campaignId, state?.projectionVersion, searchQuery]);
  useEffect(() => {
    if (!state?.campaign || view !== 'graph') return;
    void window.campaign.command({ action: 'graph' }).then(reply => {
      if (reply.ok && reply.data && typeof reply.data === 'object') {
        const data = reply.data as { nodes?: { noteId: string }[]; edges?: Array<{ source: string; target: string; occurrences: number }> };
        setGraphData({ nodes: data.nodes ?? [], edges: data.edges ?? [] });
      }
    });
  }, [view, state?.campaign?.campaignId, state?.projectionVersion]);
  ...
  function openAssistant(context: AiContextSelection = { kind: 'campaign' }) {
    setAiContext(context);
    void command({ action: 'view', view: 'assistant' });
  }
  async function chooseBoard(noteId: string, excerpt?: string) {
  ...
  if (state?.campaign) commands.push(
    ...
    ...navigation.map(n => ({ id: n.id, label: `Apri ${n.label}`, kind: 'azione' as const, run: () => n.id === 'assistant' ? openAssistant() : void command({ action: 'view', view: n.id }) })),
  ...
  return <div className="app" style={{ '--sidebar-width': `${panels.sidebarWidth}px`, '--inspector-width': `${panels.inspectorWidth}px` } as CSSProperties}>
    ...
    {boardTransfer && <section className="notice global-notice board-picker" role="dialog" aria-label="Scegli board"><strong>{boardTransfer.kind === 'character-token' ? `Crea un token collegato a ${stem(boardTransfer.noteId)}` : `Porta ${boardTransfer.excerpt === undefined ? 'la nota' : 'l’estratto'} sulla board`}</strong><div className="actions">{boardTransfer.boards.map(board => <button key={board.path} onClick={() => void sendToBoard(board.path)}>{board.title}</button>)}<button onClick={() => setBoardTransfer(undefined)}>Annulla</button></div></section>}
    ...
    {!state?.campaign ? <main className="welcome"><span className="eyebrow">IL TUO MONDO, UNA PAGINA ALLA VOLTA</span><h1>Un posto per<br/>le tue storie.</h1><p>Apri una cartella di note Markdown.<br/>Le tue campagne restano sul tuo computer.</p><button className="primary" onClick={() => void command({ action: 'choose' })}>Apri cartella… <Icon name="forward" /></button>{!!state?.preferences.recent.length && <section className="recent-campaigns"><h2>Campagne recenti</h2>{state.preferences.recent.map(item => <button key={item.campaignId} onClick={() => void command({ action: 'recent', campaignId: item.campaignId })}><Icon name="folder" /><span><strong>{item.name}</strong><small>{item.path}</small></span><Icon name="forward" /></button>)}</section>}</main> : <main className="workspace">
      <nav className="rail" aria-label="Navigazione principale">{navigation.map(n => <button className={view === n.id ? 'active' : ''} key={n.id} aria-label={n.label} aria-current={view === n.id ? 'page' : undefined} title={n.label} onClick={() => n.id === 'assistant' ? openAssistant() : void command({ action: 'view', view: n.id })}><Icon name={n.icon} /><span>{n.label}</span></button>)}</nav>
      {sidebarVisible && <><aside className="vault-sidebar"><div className="sidebar-heading"><h1>{view === 'recent' ? 'Recenti' : view === 'favorites' ? 'Preferiti' : 'La tua campagna'}</h1>{view !== 'boards' && <button className="icon-button" aria-label="Nascondi sidebar" onClick={() => updatePanels({ sidebarCollapsed: true })}><Icon name="panel" size={16} /></button>}</div>
        ...
        <nav className="vault-tree" aria-label="Note della campagna">{visible.map(entry => <div key={entry.id} className={`tree-row ${entry.id === ui.selectedFolder || entry.id === doc?.noteId ? 'selected' : ''} ${dropTarget === entry.id ? 'drop-target' : ''}`} style={{ '--depth': entry.id.split('/').length - 1 } as CSSProperties} draggable={!busy} onDragStart={e => { e.dataTransfer.setData('application/x-campaign-resource', entry.id); if (entry.kind === 'note') { e.dataTransfer.setData('application/x-campaign-note', entry.id); e.dataTransfer.effectAllowed = 'copyMove'; } else e.dataTransfer.effectAllowed = 'move'; }} onDragEnd={() => setDropTarget(undefined)} onDragOver={entry.kind === 'folder' ? e => { e.preventDefault(); e.dataTransfer.dropEffect = 'move'; setDropTarget(entry.id); } : undefined} onDrop={entry.kind === 'folder' ? e => drop(e, entry.id) : undefined}>
          ...
          <details className="row-menu"><summary aria-label={`Azioni ${entry.id}`} title={`Azioni ${entry.id}`}><Icon name="more" size={15} /></summary><div className="row-menu-content">{entry.kind === 'note' && <><button onClick={() => void command({ action: 'note', noteId: entry.id, newTab: true })}>Apri in nuova tab</button><button onClick={() => void chooseBoard(entry.id)}>Porta sulla board…</button><button onClick={() => void chooseBoardForCharacterToken(entry.id)}>Crea token da questa nota…</button><button onClick={() => void command({ action: 'favorite', noteId: entry.id })}>{ui.favorites.includes(entry.id) ? 'Rimuovi dai preferiti' : 'Aggiungi ai preferiti'}</button></>}<button onClick={() => begin('rename', entry.id)}>Rinomina</button><button onClick={() => begin('move', entry.id)}>Sposta…</button><button onClick={() => void command({ action: 'trashResource', id: entry.id })}>Cestina</button></div></details>
      ...
      <section className="center"><div className="tabs" style={['boards', 'live', 'assistant'].includes(view) ? { display: 'none' } : undefined} role="tablist" aria-label="Note aperte">{!sidebarVisible && <button className="icon-button" aria-label="Mostra sidebar" onClick={() => updatePanels({ sidebarCollapsed: false })}><Icon name="folder" /></button>}{state.tabs?.map((tab, index) => <div className={`tab ${tab.id === state.activeTabId ? 'active' : ''}`} key={tab.id} draggable onDragStart={e => e.dataTransfer.setData('application/x-campaign-tab', tab.id)} onDragOver={e => e.preventDefault()} onDrop={e => { e.preventDefault(); const id = e.dataTransfer.getData('application/x-campaign-tab'); if (state.tabs.some(t => t.id === id)) void command({ action: 'reorderTab', id, index }); }}><button role="tab" aria-selected={tab.id === state.activeTabId} onClick={() => void command({ action: 'activateTab', id: tab.id })} onKeyDown={e => { if (e.altKey && ['ArrowLeft', 'ArrowRight'].includes(e.key)) { e.preventDefault(); void command({ action: 'reorderTab', id: tab.id, index: Math.max(0, Math.min(state.tabs.length - 1, index + (e.key === 'ArrowRight' ? 1 : -1))) }); } }}><Icon name="note" size={14} /><span>{title(tab.document)}</span>{tab.document && tab.document.state !== 'clean' && <span className="unsaved" aria-label="Modifiche non salvate">•</span>}</button><button className="tab-close" aria-label={`Chiudi tab ${title(tab.document)}`} onClick={() => void command({ action: 'closeTab', id: tab.id })}><Icon name="close" size={12} /></button></div>)}<button className="icon-button" aria-label="Nuova nota in nuova tab" onClick={() => void command({ action: 'newNote', newTab: true })}><Icon name="plus" size={16} /></button><span className="tab-spacer" />{!inspectorVisible && <button className="icon-button" aria-label="Mostra inspector" onClick={() => updatePanels({ inspectorCollapsed: false })}><Icon name="panel" /></button>}</div>
        ...
        {view === 'boards' ? <BoardWorkspace command={window.campaign.command} noteIds={noteIds} characterTokenRequest={characterTokenRequest} onCharacterTokenRequestHandled={clearCharacterTokenRequest} activeBoardPath={activeBoardPath} onActiveBoardPathChange={setActiveBoardPath} /> : view === 'live' ? <LiveSessionView live={live} command={window.campaign.command} /> : view === 'assistant' ? <AiWorkspace state={ai} command={window.campaign.command} context={aiContext} onContextChange={setAiContext} onOpenSettings={() => void command({ action: 'view', view: 'settings' })} onOpenNote={noteId => void command({ action: 'note', noteId })} folders={folders.map(folder => folder.id)} /> : ['notes', 'recent', 'favorites'].includes(view) ? doc ? <>
          ...
          <div className="editor-tools"><button aria-pressed={reading} onClick={() => void toggleReading()} disabled={busy}>{reading ? 'Modifica' : 'Leggi'} <kbd>Ctrl E</kbd></button>{!reading && <><button onClick={() => undo()} disabled={busy}>Annulla modifica</button><button onClick={() => undo(true)} disabled={busy}>Ripeti modifica</button>{!doc.draft && selectedText.trim() && <><button onMouseDown={event => event.preventDefault()} onClick={() => void chooseBoard(doc.noteId, selectedText)}>Porta sulla board…</button><button onMouseDown={event => event.preventDefault()} onClick={() => openAssistant({ kind: 'selection', noteId: doc.noteId, selection: selectedText })}>Chiedi all’IA</button></>}</>}<span>{reading ? 'Lettura' : 'Markdown'}</span></div>
